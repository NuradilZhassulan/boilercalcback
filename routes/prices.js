...
router.post('/', async (req, res) => {
    ...
    // // Проверка на уникальность мощности
    // const existingPrice = await BoilerPrice.findOne({ power1 });
    // if (existingPrice) {
    //   return res.status(400).json({ message: 'Цена для данной мощности уже существует.' });
    // }
...
router.patch('/:id', async (req, res) => {
  try {
    const updatedPrice = await BoilerPrice.findByIdAndUpdate(req.params.id, req.body, { new: true });
